...
export default function NotificationBell() {
  ...
  useEffect(() => {
    ...
    const setupNotifications = async () => {
      ...
      // 3. Ativar REALTIME (O Segredo para aparecer instantaneamente)
  ...
  const fetchNotifications = async (uid) => {
    const { data, error } = await supabase
      ...
      .limit(50); // Aumentei o limite para garantir que vês tudo
            ...
            {notifications.length > 0 ? (
              notifications.map((n) => (
                <div key={n.id} className="p-4 border-b border-white/5 hover:bg-white/5 transition-colors group flex gap-3">
                   {n.image_url ? (
                    <img src={n.image_url} alt="Capa" className="w-10 h-14 object-cover rounded bg-white/10" />
                  ) : (
