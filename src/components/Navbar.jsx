...
export default function Navbar() {
  // Correção: define 'authLoading' para não dar erro na renderização
  const { user, role, loading: authLoading } = useAuthRole();
  const isAdmin = role === "admin" || role === "mod";
  
  const [pendingCount, setPendingCount] = useState(0);

  // Verifica notificações (Reports + Sugestões)
  useEffect(() => {
    ...
    <>
      <nav className="fixed top-0 left-0 w-full bg-black/80 backdrop-blur-md z-50 px-4 sm:px-6 h-14 flex items-center justify-between border-b border-gray-800">
        <div className="flex items-center gap-4 sm:gap-6">
          <Link href="/" className="text-2xl font-bold text-red-600 tracking-tighter">LusoStream</Link>
            ...
            {showDropdown && results.length > 0 && (
              <div className="absolute right-0 mt-2 w-[320px] max-h-72 overflow-auto bg-gray-900 rounded shadow-lg z-50 border border-gray-700">
                {results.slice(0, 8).map((item) => (
                  <div key={item.id + item.type} onClick={() => { router.push(item.type === "movie" ? `/movies/${item.id}` : `/series/${item.id}`); setShowDropdown(false); }} className="flex items-center gap-3 px-3 py-2 hover:bg-gray-800 cursor-pointer border-b border-gray-800/50 last:border-0">
                ...
                <div className="p-2 sticky bottom-0 bg-gray-900 border-t border-gray-700">
                  <button onClick={() => handleSearchSubmit()} className="w-full bg-red-600 py-2 rounded text-sm hover:bg-red-700 font-semibold text-white transition">Ver todos</button>
                </div>
              </div>
            )}
          </div>
          <div className="flex items-center gap-3">
            {authLoading ? <div className="w-6 h-6 border-2 border-red-600 border-t-transparent rounded-full animate-spin"></div> : user ? (
              <>
            ...
            ) : <Link href="/auth" className="bg-red-600 px-4 py-1.5 rounded hover:bg-red-700 text-sm font-semibold transition">Entrar</Link>}
          </div>
        </div>
        <button type="button" className="md:hidden w-10 h-10 flex items-center justify-center rounded bg-gray-900/70 border border-gray-700" onClick={() => setMobileOpen((s) => !s)}>{mobileOpen ? "✕" : "☰"}</button>
      </nav>
      {mobileOpen && (
        <div className="md:hidden fixed left-0 right-0 bottom-0 bg-black/95 backdrop-blur-xl" style={{ top: NAV_HEIGHT, zIndex: 9999 }}>
          ...
          <div className="flex flex-col mt-6 space-y-1">
            ...
            {isAdmin && <button onClick={() => goMobile("/admin")} className="text-left py-3 px-6 border-b border-gray-800 text-yellow-300 font-bold flex justify-between items-center">Painel Admin {pendingCount > 0 && <span className="bg-red-600 text-white text-xs px-2 py-0.5 rounded-full">{pendingCount} novos</span>}</button>}
          </div>
