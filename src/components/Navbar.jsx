...
export default function Navbar() {
  // 👇 AQUI ESTÁ A CORREÇÃO: "loading: authLoading" define a variável que faltava
  const { user, role, loading: authLoading } = useAuthRole();
  const isAdmin = role === "admin" || role === "mod";
  
  // Lógica das Notificações (Bolinha Vermelha)
  ...
  // --- LÓGICA DA BARRA DE PESQUISA (MANTIDA IGUAL) ---
  ...
  async function fetchResults(query) {
    try {
      const [movieRes, tvRes] = await Promise.all([
        fetch(`https://api.themoviedb.org/3/search/movie?api_key=${API_KEY}&language=pt-BR&query=${encodeURIComponent(query)}`),
        fetch(`https://api.themoviedb.org/3/search/tv?api_key=${API_KEY}&language=pt-BR&query=${encodeURIComponent(query)}`),
      ]);
        ...
        <div className="hidden md:flex items-center gap-4">
          ...
          <div className="relative" ref={dropdownRef}>
            <form onSubmit={handleSearchSubmit} className="flex">
              <input
                className="w-40 sm:w-56 md:w-72 p-2 rounded-l bg-gray-800 placeholder-gray-400 outline-none text-sm text-white focus:ring-2 focus:ring-red-600 transition-all"
                placeholder="Pesquisar..."
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                onFocus={() => setShowDropdown(true)}
              />
              <button type="submit" className="bg-red-600 px-3 rounded-r hover:bg-red-700 transition">
                🔍
              </button>
            </form>
              ...
              <div className="absolute right-0 mt-2 w-[320px] max-h-72 overflow-auto bg-gray-900 rounded shadow-lg z-50 border border-gray-700">
                {results.slice(0, 8).map((item) => (
                  <div
                    ...
                    className="flex items-center gap-3 px-3 py-2 hover:bg-gray-800 cursor-pointer border-b border-gray-800/50 last:border-0"
                  >
                    <img
                      src={item.poster_path ? `https://image.tmdb.org/t/p/w92${item.poster_path}` : "/no-image.jpg"}
                ...
                <div className="p-2 sticky bottom-0 bg-gray-900 border-t border-gray-700">
                  <button
                    ...
                    className="w-full bg-red-600 py-2 rounded text-sm hover:bg-red-700 font-semibold text-white transition"
                  >
          ...
          <div className="flex items-center gap-3">
            {authLoading ? (
              ...
              <Link
                href="/auth"
                className="bg-red-600 px-4 py-1.5 rounded hover:bg-red-700 text-sm font-semibold transition shadow-red-900/20 shadow-lg"
              >
                Entrar
              </Link>
            )}
        ...
        >
          <form onSubmit={handleSearchSubmit} className="flex px-4 pt-4 gap-2">
            <input
              ...
              onChange={(e) => setSearchTerm(e.target.value)}
            />
            <button type="submit" className="bg-red-600 px-4 rounded hover:bg-red-700 text-sm font-bold">
              🔍
            </button>
          </form>
          ...
          <div className="absolute bottom-10 left-4 right-4">
            {authLoading ? (
              ...
              <div className="flex gap-3">
                <button onClick={() => goMobile("/account")} className="flex-1 bg-gray-800 py-3 rounded text-center font-semibold border border-gray-700">
                  Minha Conta
                </button>
                <button onClick={handleLogout} className="flex-1 bg-red-600 py-3 rounded text-center font-semibold text-white">
                  Sair
                </button>
              </div>
            ) : (
              <button onClick={() => goMobile("/auth")} className="w-full bg-red-600 py-3 rounded text-center font-bold text-white shadow-lg shadow-red-900/30">
                Entrar / Criar Conta
              </button>
            )}
          </div>
